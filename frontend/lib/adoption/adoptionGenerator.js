...
export class AdoptionGenerator {
  constructor(options = {}) {
    ...
    this.onBatchFinalized =
      typeof this.options.onBatchFinalized === "function"
        ? this.options.onBatchFinalized
        : null;
    this.persistCat =
      typeof this.options.persistCat === "function"
    ...
    this.onGenerationStart =
      typeof this.options.onGenerationStart === "function"
        ? this.options.onGenerationStart
        : null;
    this.onRevealComplete =
      typeof this.options.onRevealComplete === "function"
    ...
    if (this.overlayCopyBigBtn) {
      this.overlayCopyBigBtn.dataset.originalText =
        this.overlayCopyBigBtn.textContent;
    }
    if (this.overlayCopyShareBtn) {
      this.overlayCopyShareBtn.dataset.originalText =
        this.overlayCopyShareBtn.textContent;
    }
    if (this.overlayOpenViewerBtn) {
      this.overlayOpenViewerBtn.disabled = true;
      this.overlayOpenViewerBtn.dataset.originalText =
        this.overlayOpenViewerBtn.textContent;
    }
    this.overlayCloseTargets = Array.from(
      document.querySelectorAll("[data-overlay-close]"),
    );
    this.spriteGalleryCloseTargets = Array.from(
      ...
      this.generateButton.addEventListener("click", () => {
        if (!this.isGenerating) {
          this.generate().catch((err) => {
            ...
            this.finishGenerationWithError(
              "Failed to generate cats. Please try again.",
            );
          });
        }
      });
      ...
      if (this.overlayOpenSpritesBtn) {
        this.overlayOpenSpritesBtn.addEventListener("click", () => {
          ...
          if (!this.spriteGalleryPreviews.length && this.currentDetailParams) {
            this.prepareSpriteGallery(
              this.currentDetailParams,
              this.detailRenderToken,
              { force: true },
            ).catch((error) =>
              console.error("Failed to prepare sprite gallery", error),
            );
          }
        });
      }
  ...
  async generate() {
    if (!this.hasActivePalettes()) {
      this.stageStatusEl.textContent =
        "Select at least one colour palette before generating.";
      return;
    }
  ...
  async runStageSequence() {
    for (const stage of this.stagePlan) {
      ...
      await this.wait(
        this.speedDurations[this.currentSpeed] + CONFIG.TIMING.BETWEEN_STAGES,
      );
    }
  }

  async animateStage(stage) {
    const tasks = this.catPlans.map((plan) =>
      this.animateStageForCat(plan, stage),
    );
  ...
  async enterRemovalPhase(stage) {
    if (this.catPlans.length <= 10) {
      if (this.catPlans.length === 10) {
        this.stageStatusEl.textContent =
          "Ten finalists locked in—continuing reveals.";
      } else {
        this.stageStatusEl.textContent =
          "Keeping the current pool—continuing reveals.";
      }
      this.removalTip.textContent =
        "Removals complete—enjoy the rest of the reveal.";
      return;
    }
  ...
  finishGeneration() {
    this.stageStatusEl.textContent =
      "Finalised ten cats! You can regenerate to try again.";
  ...
  async handleBatchFinalized() {
    ...
    const cats = finalPlans.map((plan, index) =>
      this.buildCatExport(plan, index),
    );
    const snapshot = this.lastSettingsSnapshot
      ? { ...this.lastSettingsSnapshot }
      : null;
  ...
  buildSettingsSnapshot(layerConfig) {
    return {
      ...
      afterlifeMode: this.afterlifeSelect
        ? this.afterlifeSelect.value
        : AFTERLIFE_DEFAULT,
  ...
  resolveAfterlifeFlags() {
    const value = this.afterlifeSelect
      ? this.afterlifeSelect.value
      : AFTERLIFE_DEFAULT;
    switch (value) {
      case "off":
  ...
  async renderCatCards() {
    ...
    await Promise.all(
      this.catPlans.map((plan) => this.drawCat(plan.state, plan.canvas)),
    );
  }
  ...
  createCatCard(plan) {
    ...
    card.addEventListener("click", (event) => {
      ...
      if (this.isGenerating || this.awaitingRemoval || !this.isRevealComplete)
        return;
  ...
  async animateTortieLayer(plan, stage) {
    ...
    for (const mask of maskOptions) {
      const tempState = this.cloneState(baseState);
      tempState.tortieLayers[stage.layerIndex] = {
        mask,
    ...
    const patternOptions = this.sampleVariations(
      "tortiePattern",
      layer.pattern,
    );
  ...
  async animateTortieSubElement(plan, stage) {
    ...
    await this.drawCat(plan.state, plan.canvas);
    plan.valueEl.textContent = this.describeTortieSubElement(
      layerIndex,
      finalLayer,
      revealed,
    );
  ...
  describeTortieLayer(stage, layer) {
    ...
    const parts = [layer.mask, layer.pattern, layer.colour].map((v) =>
      this.formatValue(v),
    );
  ...
  buildRenderParams(state) {
    ...
    return {
      ...
      whitePatches:
        state.whitePatches !== "none" ? state.whitePatches : undefined,
      points: state.points !== "none" ? state.points : undefined,
      whitePatchesTint:
        state.whitePatchesTint !== "none" ? state.whitePatchesTint : undefined,
  ...
  buildCatExport(plan, index = 0) {
    ...
    const label =
      plan?.labelEl?.textContent?.trim() || `Cat ${plan.index ?? index + 1}`;
    return {
      label,
      index: index,
  ...
  getWhitePatchTintOptions(mode = this.whitePatchColourMode) {
    ...
    const raw = spriteMapper.getWhitePatchColourOptions(mode, includeModes) || [
      "none",
    ];
    const seen = new Set();
    const ordered = [];
  ...
  updateGenerateButtonState() {
    ...
    this.generateButton.textContent = this.isGenerating
      ? "Rolling…"
      : "Generate Adoption";
  }
  ...
  ensureLayerDepth(params, layerConfig) {
    ...
    const accessoriesPool = (
      this.parameterOptions?.accessory ||
      spriteMapper.getAccessories() ||
      []
    ).filter((item) => item && item !== "none");
    const scarsPool = (
      this.parameterOptions?.scar ||
      spriteMapper.getScars() ||
      []
    ).filter((item) => item && item !== "none");
    const tortieMaskPool = (
      this.parameterOptions?.tortieMask ||
      spriteMapper.getTortieMasks() ||
      []
    ).filter((item) => item && item !== "none");
    const tortiePatternPool = (
      this.parameterOptions?.tortiePattern ||
      spriteMapper.getPeltNames() ||
      []
    ).filter((item) => item && item !== "none");
    const tortieColourPool = (
      this.parameterOptions?.tortieColour ||
    ...
    if (
      layerConfig.tortieCount > 0 &&
      tortieMaskPool.length &&
      tortiePatternPool.length &&
      tortieColourPool.length
    ) {
      params.tortie = [];
  ...
  populateDetailTable(plan, params) {
    ...
    const eyesDisplay =
      params.eyeColour2 && params.eyeColour2 !== params.eyeColour
    ...
    const maxTortie = Math.max(
      tortieSlots.length,
      plan.tortieSlots?.length || 0,
    );
    ...
    addRow("Shading", params.shading ? "Yes" : "No", {
      formatted: true,
      always: true,
    });
    addRow("Reverse", params.reverse ? "Yes" : "No", {
      formatted: true,
      always: true,
    });
    addRow("Dark Forest", params.darkForest ? "Yes" : "No", {
      formatted: true,
      always: true,
    });
    addRow("Star Clan", params.dead ? "Yes" : "No", {
      formatted: true,
      always: true,
    });
  ...
  async buildDetailShareUrl(plan, params) {
    try {
      ...
      if (this.persistCat) {
        try {
          const persistResult = await this.persistCat(sharedPayload, {
            plan,
            params,
          });
          ...
          console.warn(
            "Failed to persist share payload, falling back to local share options.",
            error,
          );
        }
      }
      ...
      if (shareSlug) {
        const builderUrl = new URL(
          `/visual-builder?share=${encodeURIComponent(shareSlug)}`,
          viewerUrl,
        );
  ...
  closeDetailOverlay(force = false) {
    ...
    this.spriteGalleryPreviews = [];
    if (this.overlayCopyBigBtn?.dataset.originalText) {
      this.overlayCopyBigBtn.textContent =
        this.overlayCopyBigBtn.dataset.originalText;
