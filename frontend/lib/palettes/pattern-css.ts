...
const CSS_GENERATORS: Record<
  PatternDefinition["type"],
  (p: PatternDefinition) => CSSPatternStyle
> = {
  tartan: tartanCss,
  gingham: ginghamCss,
...
export function patternToCssBackground(
  ...
  if (!generator) {
    console.warn(
      `[pattern-css] No CSS generator for pattern type "${pattern.type}", falling back to flat color`,
    );
