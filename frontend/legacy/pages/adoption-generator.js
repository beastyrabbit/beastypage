...
class AdoptionGenerator {
    constructor() {
        ...
        this.overlaySpritesGrid = document.getElementById('detailSpritesGrid');
    ...
    async generateRandomParams(layerConfig) {
        ...
        const enableDarkForest = this.resolveRandomToggle(this.darkForestSelect, 0.1);
        const enableDead = this.resolveRandomToggle(this.deadSelect, 0.1);
    ...
    resolveRandomToggle(select, defaultChance) {
        if (!select) return false;
        const value = select.value;
        if (value === 'on') return true;
        if (value === 'off') return false;
        const chance = value === 'random25' ? 0.25 : defaultChance;
    ...
    async animateSimpleParameter(plan, stage) {
        ...
        plan.valueEl.textContent = this.describeStageValue(stage, finalValue, plan);
    }
    ...
    describeStageValue(stage, value, plan) {
    ...
    async renderDetailOverlay(plan, token) {
        ...
        try {
            ...
            await this.renderDetailSprites(params, token);
    ...
    async renderDetailSprites(params, token) {
        if (!this.overlaySpritesGrid) return;
        this.overlaySpritesGrid.innerHTML = '';
        const tasks = [];

        for (const spriteNumber of this.spriteOptions) {
            const spriteItem = document.createElement('div');
            spriteItem.className = 'overlay-sprite-item';
            if (spriteNumber === params.spriteNumber) {
                spriteItem.classList.add('active');
            }

            const name = this.spriteNames[spriteNumber] || `Sprite ${spriteNumber}`;
            spriteItem.innerHTML = `
                <div class="sprite-header">${name}</div>
                <div class="sprite-canvas-wrap">
                    <canvas width="120" height="120" class="sprite-canvas"></canvas>
                </div>
            ...
            `;
            this.overlaySpritesGrid.appendChild(spriteItem);

            const canvas = spriteItem.querySelector('.sprite-canvas');
            const copyButtons = spriteItem.querySelectorAll('.sprite-copy');
    ...
    async buildDetailShareUrl(plan, params) {
        try {
            const accessorySlots = Array.isArray(plan.accessorySlots)
                ? [...plan.accessorySlots]
                : [];
            const scarSlots = Array.isArray(plan.scarSlots)
                ? [...plan.scarSlots]
                : [];
            const tortieSlots = Array.isArray(plan.tortieSlots)
...
function startAdoptionGenerator() {
    new AdoptionGenerator();
}

if (document.readyState === 'loading') {
    document.addEventListener('DOMContentLoaded', () => {
        setTimeout(startAdoptionGenerator, 60);
    });
} else {
    setTimeout(startAdoptionGenerator, 60);
}
