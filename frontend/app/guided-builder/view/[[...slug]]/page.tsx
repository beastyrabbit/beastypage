...
export const metadata: Metadata = {
  ...
  description:
    "Review each step from a guided builder session and export the final sprite.",
};
...
export default async function GuidedBuilderViewerPage({
  ...
  const slug =
    Array.isArray(resolvedParams.slug) && resolvedParams.slug.length > 0
      ...
      : null;
  const encoded =
    typeof searchParams?.data === "string" ? searchParams?.data : null;
    ...
    <main className="mx-auto flex w-full max-w-6xl flex-col gap-10 px-4 py-12 sm:px-6 lg:px-8">
      <section className="rounded-3xl border border-slate-800 bg-slate-950/70 p-6">
        ...
        <p className="mt-2 max-w-2xl text-sm text-neutral-300">
          Explore each saved step, copy the build, and export the final sprite.
          Use the sidebar to jump between major trait decisions.
        </p>
      </section>
