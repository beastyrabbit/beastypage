...
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    return NextResponse.json({ slug: record.slug, config: record.config, createdAt: record.createdAt, updatedAt: record.updatedAt });
  } catch (error) {
    console.error("Failed to fetch dash settings", error);
...
export async function POST(request: NextRequest) {
  ...
  try { body = await request.json(); }
    ...
    console.error("Failed to parse dash settings payload", error);
    return NextResponse.json({ error: "Invalid JSON body" }, { status: 400 });
  }

  const payload = (body as { config?: unknown; slug?: string }) ?? {};
  if (!payload.config) return NextResponse.json({ error: "Missing config" }, { status: 400 });

  try {
    const convex = new ConvexHttpClient(convexUrl);
    const result = await convex.mutation(api.dashSettings.save, {
    ...
    console.error("Failed to save dash settings", error);
