  ...
  return (
    <div
      className={cn(
        ...
        selected
          ? "border-amber-400 shadow-[0_0_14px_rgba(245,158,11,0.35)]"
          : "border-slate-800",
      )}
    >
      {loading && (
...
export function VisualBuilderClient({
  ...
  const [expandedMarking, setExpandedMarking] = useState<
    "white" | "points" | "vitiligo" | "tint" | null
  >(null);
  const [expandedAccessoryGroup, setExpandedAccessoryGroup] = useState<
    string | null
  >(null);
  const [expandedScarGroup, setExpandedScarGroup] = useState<string | null>(
    null,
  );
  const [expandedSkinGroup, setExpandedSkinGroup] = useState<
    "skin" | "tint" | null
  >(null);
  ...
  const unlockShare = useCallback(() => {
    ...
    return true;
  }, [lockedShareSlug, defaultCreatorName]);
  ...
  useEffect(() => {
    ...
    setShareStale(false);
  }, [ensureTortieSync, initialCat, defaultCreatorName]);
  ...
  const getPaletteForMode = useCallback(
    (mode: PaletteMode) => {
      ...
      const options =
        mapperInstance.getColourOptions?.(mode, includeBase) ??
        mapperInstance.getColours();
      return options;
    },
    [mapper],
  );
  ...
  const computeDefaultTortieLayer = useCallback(
    (index: number, base: CatParams): TortieLayer => {
      ...
      const defaultPattern = pelts.includes("SingleColour")
        ? "SingleColour"
        : (pelts[0] ?? base.peltName ?? "SingleColour");
      const defaultColour = colours.includes("GINGER")
        ? "GINGER"
        : (colours[0] ?? base.colour ?? "GINGER");
      const maskPreferences = ["ONE", "TWO", "THREE", "FOUR"];
      const preferred = maskPreferences[index];
      const defaultMask =
        preferred && masks.includes(preferred)
          ? preferred
          : (masks[0] ?? "ONE");
      return {
  ...
  const handleBasePaletteChange = useCallback(
    (value: PaletteMode) => {
  ...
  const handleTortiePaletteChange = useCallback(
    (value: PaletteMode) => {
    ...
    >
      <header className="space-y-1">
        ...
        <p className="text-sm text-neutral-300">
          Pick the primary coat colour, including experimental palettes.
        </p>
        <PaletteDropdown
          mode={experimentalColourMode}
          onChange={handleBasePaletteChange}
        />
      </header>
  ...
  const renderPatternSection = () => (
    ...
    >
      <header className="space-y-1">
        ...
        <p className="text-sm text-neutral-300">
          Change the base coat style that other markings layer on top of.
        </p>
      </header>
      <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4">
  ...
  const handleTortieToggle = useCallback(
    (enabled: boolean) => {
      ...
      const firstLayer =
        tortieLayers[0] ?? computeDefaultTortieLayer(0, params);
  ...
  const handleAddLayer = useCallback(() => {
    ...
    const next = [
      ...tortieLayers,
      computeDefaultTortieLayer(tortieLayers.length, params),
    ];
  ...
  const handleUpdateLayer = useCallback(
    (index: number, mutator: (draft: TortieLayer) => void) => {
      ...
      const draftLayer = {
        ...(next[index] ?? computeDefaultTortieLayer(index, params)),
      };
      mutator(draftLayer);
      ...
      >
        <header className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
          <div>
            ...
            <p className="text-sm text-neutral-300">
              Enable layered tortie colours and customize each mask
              individually.
            </p>
          </div>
        ...
        {hasLayers ? (
          <div className="space-y-6">
            {currentLayers.map((layer, layerIndex) => {
              ...
              if (expanded) {
                const patternContent = (
                  <div className="mt-4 grid gap-3 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4">
                    {(currentOptions?.pelts ?? []).map((pattern) => {
                      ...
                      const selected =
                        (layer.pattern ?? currentParams.peltName) === pattern;
                      return (
                        <button
                  ...
                  <div className="space-y-3">
                    <div className="flex flex-wrap items-center gap-2 text-xs sm:text-sm">
                      <PaletteDropdown
                        ...
                        onChange={handleTortiePaletteChange}
                      />
                    </div>
                    <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4">
                      {tortiePalette.map((colour) => {
                        ...
                        const selected =
                          (layer.colour ?? currentParams.colour) === colour;
                        const badgeColour = getColourSwatch(
                          colour,
                ...
                const subSections = [
                  {
                    ...
                    title: "Pattern",
                    summary: formatName(
                      layer.pattern ?? currentParams.peltName,
                    ),
                  ...
                  },
                  {
                    id: "colour",
                    title: "Colour",
                    summary: formatName(layer.colour ?? currentParams.colour),
                    content: colourContent,
                  },
                  {
                    id: "mask",
                    title: "Mask",
                    summary: formatName(layer.mask ?? "ONE"),
                    content: maskContent,
                  },
                ] as const;
                ...
                const activeSub =
                  storedSub === undefined ? "colour" : storedSub;
                ...
                detailContent = (
                  <div className="space-y-3">
                    {subSections.map((sub) => {
                      ...
                      return (
                        <div
                          key={`${layerPrefix}-${sub.id}`}
                          className="space-y-3 rounded-xl border border-slate-800/60 bg-slate-900/50 p-4"
                        >
                          <button
                            type="button"
                            onClick={() =>
              ...
              return (
                <div
                  key={layerPrefix}
                  className="space-y-4 rounded-2xl border border-slate-800/70 bg-slate-900/60 p-5"
                >
                  <div className="flex items-center justify-between gap-3">
                    <button
                      type="button"
                      onClick={() =>
                        setExpandedLayer(expanded ? null : layerIndex)
                      }
          ...
          <div className="rounded-2xl border border-slate-800/70 bg-slate-900/40 px-4 py-6 text-sm text-neutral-300">
            Tortie layering is currently disabled. Toggle it on to add up to six
            independent layers with their own colours and masks.
          </div>
        )}
      </section>
    );
  ...
  const renderEyesSection = () => (
    ...
    >
      <header className="space-y-1">
        ...
        <p className="text-sm text-neutral-300">
          Select the primary and optional secondary eye colours.
        </p>
      </header>
              ...
              <button
                ...
                className={cn(
                  ...
                  params.eyeColour === eye
                    ...
                    : "text-neutral-200 hover:border-amber-300/70",
                )}
                onClick={() =>
                  updateParams(
                    (draft) => {
  ...
  const markingsSection = useMemo(() => {
    ...
    const currentParams = viewParams;
    const currentOptions = viewOptions;
    const whiteTintChoices = mapperInstance?.getWhitePatchColourOptions(
      "all",
    ...
    const whitePatchChoices = [
      ...
      ...(currentOptions?.whitePatches ?? []).map((value) => ({
        value,
    ...
    const pointChoices = [
      { value: null, label: "None" },
      ...(currentOptions?.points ?? []).map((value) => ({
        value,
        label: formatName(value),
      })),
    ];
    ...
    const vitiligoChoices = [
      { value: null, label: "None" },
      ...(currentOptions?.vitiligo ?? []).map((value) => ({
        value,
        label: formatName(value),
      })),
    ];
    ...
    const renderPreviewOption = (
      ...
      >
        ...
        <p
          className={cn(
            "mt-2 text-xs",
            selected ? "font-semibold text-amber-100" : "text-neutral-300",
          )}
        >
          {label}
        </p>
    ...
    const groups: {
    ...
    }[] = [
      {
        id: "white",
        title: "White patches",
        summary: currentParams.whitePatches
          ? formatName(currentParams.whitePatches)
          : "None",
        options: [
          ...whitePatchChoices.map(({ value, label }) => {
            ...
            return {
              ...
              label,
              selected: isNone
                ? !currentParams.whitePatches
                : currentParams.whitePatches === value,
              mutate: (draft: CatParams) => {
                draft.whitePatches = isNone ? undefined : (value as string);
              },
              traitInfo: {
                trait_type: "white_patches",
                value: value ?? "none",
              },
            };
          }),
      ...
      },
      {
        id: "points",
        title: "Points",
        summary: currentParams.points
          ? formatName(currentParams.points)
          : "None",
        options: [
          ...pointChoices.map(({ value, label }) => {
            ...
            return {
              ...
              label,
              selected: isNone
                ? !currentParams.points
                : currentParams.points === value,
              mutate: (draft: CatParams) => {
      ...
      },
      {
        id: "vitiligo",
        title: "Vitiligo",
        summary: currentParams.vitiligo
          ? formatName(currentParams.vitiligo)
          : "None",
        options: [
          ...vitiligoChoices.map(({ value, label }) => {
            ...
            return {
              ...
              label,
              selected: isNone
                ? !currentParams.vitiligo
                : currentParams.vitiligo === value,
              mutate: (draft: CatParams) => {
      ...
      },
      {
        ...
        summary:
          currentParams.whitePatchesTint &&
          currentParams.whitePatchesTint !== "none"
            ? formatName(currentParams.whitePatchesTint)
            : "None",
        options: whiteTintChoices.map((option) => {
          const selected =
            (currentParams.whitePatchesTint ?? "none") === option;
          const badge =
            option !== "none" ? (
              <span
                key={`tint-swatch-${option}`}
                className="block size-4 rounded-full border border-slate-900/70 shadow-inner"
                style={{
                  background: getColourSwatch(option, spriteMapperRef.current),
                }}
              />
            ) : undefined;
          return {
    ...
    return (
      ...
      >
        <header className="space-y-1">
          ...
          <p className="text-sm text-neutral-300">
            Add white patches, points, vitiligo, and tint adjustments.
          </p>
        </header>
        {groups.map((group) => {
  ...
  const renderMarkingsSection = useCallback(
    () => markingsSection,
    [markingsSection],
  );
  const renderSkinSection = () => {
    ...
    const groups: {
      ...
      },
      {
        ...
        title: "Overall tint",
        summary:
          params.tint && params.tint !== "none"
            ? formatName(params.tint)
            : "None",
        content: (
          <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4">
            <button
    ...
    return (
      ...
      >
        <header className="space-y-1">
          ...
          <p className="text-sm text-neutral-300">
            Adjust the skin tone, global tint, and shading options.
          </p>
        </header>
            ...
            return (
              ...
              >
                <button
                  type="button"
                  onClick={() =>
                    setExpandedSkinGroup(expanded ? null : group.id)
                  }
                  className="flex w-full items-center justify-between rounded-xl border border-slate-800/70 bg-slate-900/70 px-4 py-3 text-left transition hover:border-amber-300/70 focus:outline-none focus:ring-2 focus:ring-amber-300/40"
                  aria-expanded={expanded}
                >
          ...
          <button
            ...
            className={cn(
              ...
              params.shading
                ...
                : "border-slate-700 bg-slate-900/60 text-neutral-200 hover:border-amber-300/70",
            )}
          >
  ...
  const accessoriesSection = useMemo(() => {
    ...
    const grouped = [
      {
        ...
        options: currentOptions?.plantAccessories ?? [],
      },
      {
        label: "Wild accessories",
        options: currentOptions?.wildAccessories ?? [],
      },
    ...
    return (
      ...
      >
        <header className="space-y-1">
          ...
          <p className="text-sm text-neutral-300">
            Stack multiple accessories from any category.
          </p>
        </header>
        {normalizedGroups.map((group) => {
          const selectedLabels = (currentParams.accessories ?? [])
            .filter(
              (entry): entry is string =>
                entry !== null && group.options.includes(entry),
            )
            .map(formatName);
          ...
          return (
            ...
            >
              <button
                type="button"
                onClick={() =>
                  setExpandedAccessoryGroup(expanded ? null : group.label)
                }
                className="flex w-full items-center justify-between rounded-xl border border-slate-800/70 bg-slate-900/70 px-4 py-3 text-left transition hover:border-amber-300/70 focus:outline-none focus:ring-2 focus:ring-amber-300/40"
                aria-expanded={expanded}
              >
                    ...
                    return (
                      ...
                      >
                        <VisualBuilderPreviewSprite
                          ...
                          size={260}
                        />
                        <p className="px-3 pb-3 text-xs text-neutral-300">
                          {label}
                        </p>
                      </button>
                    );
    ...
    expandedAccessoryGroup,
    toggleAccessory,
    viewOptions,
    viewParams,
    previewSpriteSharedProps,
  ]);
  const renderAccessoriesSection = useCallback(
    () => accessoriesSection,
    [accessoriesSection],
  );
  ...
  const scarsSection = useMemo(() => {
    ...
    const groups = [
      ...
      {
        label: "Environmental",
        options: currentOptions?.scarEnvironmental ?? [],
      },
    ];
    ...
    return (
      ...
      >
        <header className="space-y-1">
          ...
          <p className="text-sm text-neutral-300">
            Highlight the cat&apos;s story with layered scars.
          </p>
        </header>
        {groups.map((group) => {
          const selectedLabels = (currentParams.scars ?? [])
            .filter(
              (entry): entry is string =>
                entry !== null && group.options.includes(entry),
            )
          ...
          return (
            ...
            >
              <button
                type="button"
                onClick={() =>
                  setExpandedScarGroup(expanded ? null : group.label)
                }
                className="flex w-full items-center justify-between rounded-xl border border-slate-800/70 bg-slate-900/70 px-4 py-3 text-left transition hover:border-amber-300/70 focus:outline-none focus:ring-2 focus:ring-amber-300/40"
                aria-expanded={expanded}
              >
    ...
    () => (
      ...
      >
        <header className="space-y-1">
          ...
          <p className="text-sm text-neutral-300">
            Choose the sprite pose and orientation.
          </p>
        </header>
  ...
  const sections: SectionDefinition[] = [
    {
      ...
      title: "Pose & sprite",
      description: "",
      render: renderPoseSection,
    },
    {
      id: "colour",
      title: "Base colour",
      description: "",
      render: renderColourSection,
    },
    {
      id: "pattern",
      title: "Pattern",
      description: "",
      render: renderPatternSection,
    },
    {
      id: "tortie",
      title: "Tortie layers",
      description: "",
      render: renderTortieSection,
    },
    { id: "eyes", title: "Eyes", description: "", render: renderEyesSection },
    {
      ...
      title: "Markings",
      description: "",
      render: renderMarkingsSection,
    },
    {
      id: "skin",
      title: "Skin & tint",
      description: "",
      render: renderSkinSection,
    },
    {
      id: "accessories",
      title: "Accessories",
      description: "",
      render: renderAccessoriesSection,
    },
    {
      id: "scars",
      title: "Scars",
  ...
  const handleReset = useCallback(() => {
    ...
    markShareDirty();
  }, [markShareDirty, unlockShare, defaultCreatorName]);

  const handleRandomize = useCallback(async () => {
    const generatorInstance = generatorRef.current;
    const randomizer = generatorInstance?.generateRandomParams?.bind(
    ...
    try {
      setRandomizing(true);
      const pickMode = () =>
        VALID_PALETTE_IDS[Math.floor(Math.random() * VALID_PALETTE_IDS.length)];
      ...
      combined.accessories = Array.isArray(random?.accessories)
        ? (random?.accessories as string[])
        : [];
      combined.scars = Array.isArray(random?.scars)
        ? (random?.scars as string[])
        : [];
      combined.accessory = combined.accessories[0] ?? undefined;
      combined.scar = combined.scars[0] ?? undefined;
      const tortie = Array.isArray(random?.tortie)
        ? (random?.tortie as TortieLayer[])
        : [];
      ...
      const nextTortiePaletteMode = determinePaletteMode(
        random?.tortiePalette,
        requestedTortiePalette,
      );
    ...
    return (
      <div className="mx-auto max-w-xl rounded-3xl border border-red-500/40 bg-red-500/10 px-6 py-12 text-center text-sm text-red-100">
        ...
        <p>
          {optionsError ??
            rendererError ??
            "Something went wrong while loading the builder. Please refresh and try again."}
        </p>
      </div>
    );
  }
    ...
    <div className="mx-auto flex w-full max-w-[1500px] flex-col gap-12 px-6 py-16 lg:px-10">
      <section className="rounded-3xl border border-amber-500/30 bg-gradient-to-br from-amber-500/15 via-slate-950 to-slate-950 p-8 text-balance shadow-[0_0_40px_rgba(245,158,11,0.15)]">
        ...
        <h1 className="mt-3 text-3xl font-semibold text-white sm:text-4xl">
          Design a cat with live sprite previews
        </h1>
      </section>
          ...
          <div className="rounded-3xl border border-slate-800 bg-slate-950/70 p-6">
            ...
            <form className="mt-5 space-y-3 text-sm">
              <div className="space-y-1">
                <label
                  htmlFor="visual-builder-cat-name"
              ...
              <div className="space-y-1">
                <label
                  htmlFor="visual-builder-creator-name"
                  className="block text-xs font-semibold uppercase tracking-wide text-neutral-400"
                >
                  Creator
                </label>
            ...
            {isShareLocked ? (
              <p className="mt-3 text-xs text-neutral-400">
                Shared cats are read-only. Change any trait to unlock editing
            ...
            <div className="mt-6 grid gap-3">
              ...
              <div className="flex flex-wrap gap-2">
                ...
                >
                  <RefreshIcon
                    size={16}
                    className={cn(randomizing && "animate-spin")}
                  />
                  Randomize
                </button>
                <button
              ...
              {builderBaseUrl && (
                <button
                  ...
                  onClick={() => {
                    ...
                    if (!opened)
                      window.alert("Enable popups to open the legacy builder.");
                  }}
        ...
        <div className="space-y-10">
          ...
          {shareStale && shareInfo && !shareBusy && (
            <p className="text-xs text-amber-200">
              You&apos;ve made changes since saving. Click “Save & Copy Link” to
