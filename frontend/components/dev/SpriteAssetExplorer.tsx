...
function canvasToDataUrl(
  ...
  try {
    ...
    const height =
      "height" in canvas &&
      typeof (canvas as OffscreenCanvas).height === "number"
...
export function SpriteAssetExplorer() {
  ...
  const [indexEntries, setIndexEntries] = useState<
    Record<string, SpriteIndexEntry>
  >({});
  ...
  const filteredKeys = useMemo(() => {
    const predicate =
      CATEGORIES.find((entry) => entry.id === category)?.predicate ??
      (() => true);
    const search = filter.trim().toLowerCase();
    return availableKeys.filter(
      (key) =>
        predicate(key) && (!search || key.toLowerCase().includes(search)),
    );
    ...
    <div className="space-y-6">
      <header className="space-y-3">
        ...
        <p className="text-sm text-neutral-300">
          Browse the sprite sheets currently bundled for the V3 renderer. Filter
          by category or search keys, then preview the 21 sprite frames directly
          in the browser.
        </p>
      </header>
        ...
        <section className="space-y-3 rounded-xl border border-slate-800 bg-slate-900/60 p-4">
          ...
          <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-3">
            {exclusiveGroups.slice(0, 9).map((group) => (
              <div
                key={group.group}
                className="rounded-lg border border-slate-800/70 bg-slate-950/50 p-3 text-xs text-neutral-300"
              >
                ...
                {group.examples.length > 0 && (
                  <div className="mt-1 text-[11px] text-neutral-400">
                    Examples:{" "}
                    {group.examples
                      .map((example) => example.replace(/\.png$/i, ""))
                      .join(", ")}
                  </div>
                )}
              </div>
          ...
          <p className="text-[11px] text-neutral-500">
            Heads-up: because these files are only on the backend, the previewer
            can’t display them yet. Promote any groups you want in the web
            bundle by copying the sheets into{" "}
      ...
      <section className="grid gap-4 rounded-xl border border-slate-800 bg-slate-900/60 p-4 md:grid-cols-4">
        <div className="md:col-span-1 space-y-4">
          <div>
            <p className="block text-xs font-semibold uppercase tracking-wide text-neutral-400">
              Category
            </p>
        ...
        <div className="md:col-span-3">
          <div className="mb-2 text-xs text-neutral-400">
            ...
            {availableKeys.length.toLocaleString()} sprite groups
          </div>
        ...
        <section className="space-y-4 rounded-xl border border-slate-800 bg-slate-900/60 p-4">
          <div className="flex flex-wrap items-center justify-between gap-3">
            <div>
              ...
              {selectedMeta && (
                <p className="text-xs text-neutral-400">
                  Sheet: {selectedMeta.spritesheet} · offset (
                  {selectedMeta.xOffset}, {selectedMeta.yOffset})
                  {selectedMeta.width
            ...
            {loadingKey && (
              <div className="text-xs text-neutral-400">Loading frames…</div>
            )}
          </div>
