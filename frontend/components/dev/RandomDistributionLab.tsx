...
const METRICS: MetricDefinition[] = [
  {
    ...
    label: "Sprite Number",
    extractor: (p) =>
      (p.spriteNumber as string | number | null | undefined) ?? "unknown",
    importance: "high",
  },
  {
    key: "peltName",
    label: "Pelt",
    extractor: (p) => p.peltName as string | null | undefined,
    importance: "high",
  },
  {
    key: "colour",
    label: "Base Colour",
    extractor: (p) => p.colour as string | null | undefined,
    importance: "high",
  },
  {
    key: "tint",
    label: "Tint",
    extractor: (p) => p.tint as string | null | undefined,
    importance: "high",
  },
  {
    key: "skinColour",
    label: "Skin Colour",
    extractor: (p) => p.skinColour as string | null | undefined,
  },
  {
    key: "eyeColour",
    label: "Eye Colour",
    extractor: (p) => p.eyeColour as string | null | undefined,
  },
  {
    key: "eyeColour2",
    label: "Second Eye Colour",
    extractor: (p) => p.eyeColour2 as string | null | undefined,
  },
  {
    key: "reverse",
    label: "Reverse Pose",
    extractor: (p) => Boolean(p.reverse),
  },
  {
    key: "shading",
    label: "Shading Enabled",
    extractor: (p) => Boolean(p.shading),
  },
  {
    key: "isTortie",
    label: "Is Tortie",
    extractor: (p) => Boolean(p.isTortie),
    importance: "high",
  },
  {
    key: "tortieCount",
  ...
  },
  {
    key: "whitePatches",
    label: "White Patches",
    extractor: (p) => (p.whitePatches as string | null | undefined) ?? "none",
    importance: "high",
  },
  {
    ...
    extractor: (p) =>
      (p.whitePatchesTint as string | null | undefined) ?? "none",
  },
  {
    key: "points",
    label: "Points",
    extractor: (p) => (p.points as string | null | undefined) ?? "none",
    importance: "high",
  },
  {
    key: "vitiligo",
    label: "Vitiligo",
    extractor: (p) => (p.vitiligo as string | null | undefined) ?? "none",
    importance: "high",
  },
  {
    key: "accessoryCount",
...
function getAccessories(params: JsonParams): string[] {
  ...
  const list = Array.isArray(params.accessories)
    ? (params.accessories as unknown[])
    : [];
...
function getTortieLayers(params: JsonParams): TortieLayer[] {
  if (Array.isArray(params.tortie) && params.tortie.length) {
    return (params.tortie as unknown[]).filter(Boolean).map((entry) => {
  ...
  if (params.isTortie && params.tortieMask) {
    return [
      {
        mask:
          typeof params.tortieMask === "string" ? params.tortieMask : undefined,
        colour:
          typeof params.tortieColour === "string"
            ? params.tortieColour
            : undefined,
        pattern:
          typeof params.tortiePattern === "string"
  ...
  if (generator === "v2") {
    ...
    while (produced < total) {
      ...
      for (let i = 0; i < slice; i += 1) {
        promises.push(
          catGenerator.generateRandomParams({ ignoreForbiddenSprites: true }),
        );
      }
...
function compileMetricRows(
  ...
  for (const value of values) {
    const baselinePercent = percentOf(
  ...
  rows.sort(
    (a, b) =>
      Math.abs(b.variantDelta) - Math.abs(a.variantDelta) ||
...
function collectIssues(bundle: SimulationBundle): IssueSummary[] {
  ...
  for (const metric of METRICS) {
    ...
    for (const value of values) {
      ...
      const variantPercent = percentOf(
        variantCounts[value],
        variant.iterations,
      );
...
function buildDistributionRows(
  ...
  const baselineCounts = baseline.metrics[metricKey] ?? {};
  const variantCounts = variant.metrics[metricKey] ?? {};
  const alternateCounts = alternate
    ? (alternate.metrics[metricKey] ?? {})
    : undefined;
  const values = new Set<string>([
    ...Object.keys(baselineCounts),
    ...Object.keys(variantCounts),
  ...
  values.forEach((value) => {
    ...
    const alternatePct =
      alternateCounts && alternate
        ? percentOf(alternateCounts[value], alternate.iterations)
        : undefined;
    rows.push({
      value,
  ...
  rows.sort(
    (a, b) => Math.abs(b.delta) - Math.abs(a.delta) || b.baseline - a.baseline,
  );
  return rows;
}
...
function buildExportSummary(
  ...
  const payload: Record<string, unknown> = {
    iterations: {
      ...
      alternate:
        includeAlternate && bundle.alternate
          ? bundle.alternate.iterations
          : undefined,
    },
  };
    ...
    const rows = buildDistributionRows(
      ...
      alternate:
        row.alternate === undefined
          ? undefined
          : Number(row.alternate.toFixed(4)),
  ...
  return (
    <div className="space-y-3 rounded-xl border border-slate-800 bg-slate-900/60 p-4">
      ...
      <div className="space-y-3">
        {rows.map((row) => (
          <div
            key={`${definition.metric.key}-${row.value}`}
            className="space-y-1"
          >
            <div className="flex items-center justify-between text-xs text-neutral-300">
              ...
              <div className="flex items-center gap-3 font-mono">
                <span className="text-sky-200">
                  {percentLabel(row.baseline)}
                </span>
                <span className="text-emerald-200">
                  {percentLabel(row.variant)}
                </span>
                {row.alternate !== undefined && (
                  <span className="text-orange-200">
                    {percentLabel(row.alternate)}
                  </span>
                )}
                <span className={`text-xs ${deltaClass(row.delta)}`}>
            ...
            <div className="relative h-3 w-full rounded-full bg-slate-800/70">
              <span
                ...
                style={{
                  width: `${Math.min(100, Math.max(0.2, row.baseline))}%`,
                }}
              ...
              <span
                ...
                style={{
                  width: `${Math.min(100, Math.max(0.2, row.variant))}%`,
                }}
              ...
              {row.alternate !== undefined && (
                <span
                  ...
                  style={{
                    width: `${Math.min(100, Math.max(0.2, row.alternate))}%`,
                  }}
...
function SecondaryMetricsPanel({
  sections,
  baselineId,
}: SecondaryMetricsPanelProps) {
  const [expanded, setExpanded] = useState(false);
      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-xs text-neutral-400">
            White patch tints, heterochromia, and other secondary attributes.
          </p>
        </div>
        <button
          type="button"
      ...
      {expanded && (
        <div className="mt-4 space-y-6">
          {sections.map((section) => (
            <div
              key={section.metric.key}
              className="space-y-2 rounded-lg border border-slate-800/70 bg-slate-950/40 p-3"
            >
              ...
              <div className="space-y-2">
                {section.rows.map((row) => (
                  <div
                    key={`${section.metric.key}-${row.value}`}
                    className="rounded-md border border-slate-800/60 bg-slate-900/60 p-3"
                  >
                    <div className="flex items-center justify-between gap-3">
                      ...
                      <span
                        className={`font-mono text-[11px] ${deltaClass(row.variantDelta)}`}
                      >
                        {formatDelta(row.variantDelta)}
                      </span>
                    </div>
                    <div className="mt-2 space-y-2">
                      {row.lines.map((line) => (
                        ...
                        >
                          <span className="w-36 text-[11px] text-neutral-400">
                            {line.label}
                          </span>
                          ...
                          {line.simId !== baselineId && (
                            <span
                              className={`w-16 text-right font-mono text-[11px] ${deltaClass(line.deltaFromBaseline)}`}
                            >
...
export function RandomDistributionLab() {
  ...
  const [iterationsInput, setIterationsInput] = useState<string>(() =>
  ...
  const handleRun = useCallback(async () => {
    ...
    try {
      ...
      if (includeAlternate) {
        const alternateMode: CountStrategy =
          countsMode === "uniform" ? "weighted" : "uniform";
  ...
  const metricSections = useMemo(() => {
    ...
    const sections = METRICS.map((metric) => {
      ...
      const rows = compileMetricRows(
        metric,
        baseline,
        sims,
        variantId,
        limit,
      ).filter((row) => row.maxPercent > 0);
      return { metric, rows };
    }).filter((section) => section.rows.length > 0);

    return {
      high: sections.filter((section) => section.metric.importance === "high"),
      secondary: sections.filter(
        (section) => section.metric.importance !== "high",
      ),
    };
  }, [results, simulations]);
  ...
  const distributionCharts = useMemo<DistributionChartDefinition[]>(() => {
    ...
    METRICS.filter((metric) => metric.importance === "high").forEach(
      (metric) => {
        const rows = buildDistributionRows(
          metric.key,
          results.baseline,
  ...
  const exportSnapshot = useMemo(() => {
    ...
    return buildExportSummary(
      results,
      includeAlternate && Boolean(results.alternate),
    );
    ...
    <div className="space-y-6">
      <header className="space-y-3">
        <h1 className="text-2xl font-semibold text-white">
          Random Distribution Lab
        </h1>
        <p className="text-sm text-neutral-300">
          Run large batches ({"10k"} / {"100k"} / custom) of random cats using
          the legacy V2 generator and the new V3 pipeline. Visualise where the
          distributions diverge so you can tune the generator before shipping
          it.
        </p>
      </header>
      ...
      <section className="grid gap-4 rounded-xl border border-slate-800 bg-slate-900/60 p-4 md:grid-cols-2">
        ...
        <div className="md:col-span-2 flex items-center justify-between">
          <div className="text-xs text-neutral-400">
            Iterations are capped at 200k to keep the browser responsive. Runs
            yield to the UI between batches so you can track progress.
          </div>
          <button
            type="button"
            onClick={handleRun}
          ...
          >
            {running
              ? "Running…"
              : `Run ${iterations.toLocaleString()} samples`}
          </button>
        </div>
      </section>
            ...
            <div className="grid gap-3 md:grid-cols-3">
              <div>
                ...
                <div className="text-base text-white">
                  {results.baseline.iterations.toLocaleString()}
                </div>
              </div>
              <div>
                <div className="text-xs uppercase tracking-wide text-neutral-400">
                ...
                <div className="text-base text-white">
                  {results.baseline.durationMs.toFixed(0)} ms
                </div>
              </div>
              <div>
                <div className="text-xs uppercase tracking-wide text-neutral-400">
                ...
                <div className="text-base text-white">
                  {results.variant.durationMs.toFixed(0)} ms
                </div>
              </div>
            </div>
          </div>
          ...
          <div className="rounded-lg border border-slate-800 bg-slate-900/70 p-4 text-sm text-neutral-300">
            <div className="mb-3 flex items-center justify-between">
              ...
              <span className="text-xs uppercase tracking-wide text-neutral-500">
                Δ ≥ {SIGNIFICANT_THRESHOLD}%
              </span>
            </div>
            {issueSummary.length === 0 ? (
              <p className="text-sm text-neutral-300">
                No metric deviates from V2 by more than {SIGNIFICANT_THRESHOLD}%
                  ...
                  >
                    ...
                    <div className="text-right text-xs text-neutral-400">
                      <div className="font-mono text-neutral-100">
                        V3 {formatPercent(issue.variant)}
                      </div>
                      <div className="font-mono">
                        V2 {formatPercent(issue.baseline)}
                      </div>
                    </div>
                    <div
                      className={`font-mono text-xs ${deltaClass(issue.delta)}`}
                    >
            ...
            <section className="space-y-4 rounded-lg border border-slate-800 bg-slate-900/70 p-4">
              <div className="flex flex-wrap items-center justify-between gap-3">
                <div>
                  ...
                  <p className="text-xs text-neutral-400">
                    Compare baseline V2 frequencies against the latest V3
                    samples.
                  </p>
                </div>
                <div className="flex items-center gap-2 text-xs">
                  ...
                  {copyStatus && (
                    <span className="text-emerald-300">{copyStatus}</span>
                  )}
                </div>
              </div>
              ...
              >
                <div className="flex items-center justify-between">
                  ...
                  <span className="text-xs text-neutral-400">
                    Top {section.rows.length} values for{" "}
                    {section.metric.label.toLowerCase()}
                  </span>
                </div>
                <div className="space-y-3">
                  {section.rows.map((row) => (
                    ...
                    >
                      <div className="flex items-center justify-between gap-3">
                        ...
                        <span
                          className={`font-mono text-xs ${deltaClass(row.variantDelta)}`}
                        >
                          {formatDelta(row.variantDelta)}
                        </span>
                      </div>
                      <div className="mt-2 space-y-2">
                        {row.lines.map((line) => (
                          <div
                            ...
                            className="flex items-center gap-3"
                          >
                            <span className="w-40 text-xs text-neutral-400">
                              {line.label}
                            </span>
                            <div className="relative h-2 flex-1 rounded-full bg-slate-800/70">
                              <span
                            ...
                            {line.simId !== baselineId && (
                              <span
                                className={`w-16 text-right font-mono text-xs ${deltaClass(line.deltaFromBaseline)}`}
                              >
                                {formatDelta(line.deltaFromBaseline)}
                              </span>
                            )}
