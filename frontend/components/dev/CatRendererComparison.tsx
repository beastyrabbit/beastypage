...
export function CatRendererComparison() {
    ...
    async (inputParams: Record<string, unknown>) => {
      ...
      try {
        ...
        const rendererResult = await renderCatV3({
          spriteNumber: Number(
            inputParams.spriteNumber ?? DEFAULT_PARAMS.spriteNumber,
          ),
          params: inputParams,
          collectLayers: true,
        });

        const v3Offscreen = await decodeImageFromDataUrl(
          rendererResult.imageDataUrl,
        );
        ...
        setDiff({
          ...
          ratio: mismatch / (CANVAS_SIZE * CANVAS_SIZE),
        });
  ...
  const handleRandomise = useCallback(async () => {
    try {
      ...
      const randomParams = await catGenerator.generateRandomParams({
  ...
  return (
    <div className="space-y-6">
      <header className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
        <div>
          <h1 className="text-2xl font-semibold text-white">
            Cat Generator V3 Parity Lab
          </h1>
          <p className="text-sm text-neutral-300">
            Compare legacy V2 (browser) output with the new FastAPI renderer.
            Randomise to explore more cats and inspect layer timings.
          </p>
        </div>
      ...
      <section className="grid gap-6 md:grid-cols-3">
        <CanvasPanel
          title="V2 Browser Renderer"
          subtitle="catGeneratorV2.js"
          canvasRef={v2CanvasRef}
          loading={loading}
        />
        <CanvasPanel
          title="V3 FastAPI Renderer"
          subtitle="backend/renderer_service"
          canvasRef={v3CanvasRef}
          loading={loading}
        />
        <CanvasPanel
          title="Pixel Diff"
          subtitle="Red indicates mismatched pixels"
      ...
      {diff && (
        <div className="grid gap-4 rounded-lg border border-slate-700 bg-slate-900/70 p-4 sm:grid-cols-3">
          ...
          <Metric
            ...
            helper={
              meta
                ? `memory pressure: ${meta.memory_pressure ? "yes" : "no"}`
                : "backend timing"
            }
          />
        </div>
...
function CanvasPanel({
  ...
  return (
    <div className="rounded-xl border border-slate-800 bg-slate-900/60 p-4">
      ...
      <div
        className="flex items-center justify-center rounded-lg bg-slate-950/50 p-4"
