...
def expect_sprites(
    path: Path | None = None, required: Iterable[str] | None = None
) -> Path:
    ...
    if not sprite_root.exists():
        raise MissingSpriteAssets(
            f"Sprite root '{sprite_root}' is missing. Configure CG3_SPRITE_ROOT or add assets."
        )
...
__all__ = ["MissingSpriteAssets", "expect_sprites"]
